...
public class NetworkObject : MonoBehaviour {
	...
	// Подпись на правильные циклы обновлений
	// Очереди на запросы само-обновления
	// Обноление по запросу со стороны
	...
	public virtual void Initialize(uint networkId = uint.MaxValue) {
		...
		Game.GetManager<Server_NetworkObjects>().OnCreate(this, networkId);
	}
	public virtual void DestroyObject() {
		Components.ForEach(x => x?.DestroyComponent());
		Game.GetManager<Server_NetworkObjects>().OnDestroy(this);
	}
